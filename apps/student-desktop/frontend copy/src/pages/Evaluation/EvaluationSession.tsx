...
export default function EvaluationSession({ stageId, onExit, onComplete }: EvaluationSessionProps) {
    ...
    ws.onmessage = (event) => {
      try {
        ...
        if (data.action === 'result') {
          ...
          // Per the manuscript: a score below 60 on ANY individual parameter triggers the scaffolding
  ...
  useEffect(() => {
    ...
    const draw = () => {
      ...
      const bodyColor = locOk ? '#3DBE64' : '#E5484D';
      ...
      if (lm.hand[0] && lm.hand[0].x !== 0) {
        ...
        const handColor = handOk ? '#3DBE64' : '#E5484D';
